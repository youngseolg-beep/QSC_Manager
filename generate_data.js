...
// 33 * 5 = 165
for(let i=1; i<=33; i++) {
    const options = i % 3 === 0 ? `[{label: '우수(5점)', val: 5}, {label: '보통(3점)', val: 3}, {label: '미흡(1점)', val: 1}, {label: '비해당(-)', val: -1}]` :
                    i % 3 === 1 ? `[{label: '준수(5점)', val: 5}, {label: '미준수(1점)', val: 1}, {label: '비해당(-)', val: -1}]` :
                                  `[{label: '우수(5점)', val: 5}, {label: '양호(3점)', val: 3}, {label: '미흡(0점)', val: 0}, {label: '비해당(-)', val: -1}]`;
    const section = i <= 11 ? '식자재 보관 및 관리' : i <= 22 ? '조리기구 및 설비 위생' : '주방 환경 및 개인위생';
    KITCHEN_SANITATION_ITEMS.push(`  { id: 'k_s_${i}', category: '주방', subCategory: '위생 및 시설-주방', section: '${section}', task: '주방 위생 및 시설 세부 점검 항목 ${i}', options: ${options}, maxScore: 5 },`);
}
...
for(let i=1; i<=163; i++) {
    ...
    KITCHEN_COOKING_ITEMS.push(`  { id: 'k_c_${i}', category: '주방', subCategory: '조리-주방', section: '${section}', task: '${section} 세부 메뉴 조리 준수 항목 ${i}', options: [{label: '준수(1점)', val: 1}, {label: '미준수(0점)', val: 0}, {label: '비해당(-)', val: -1}], maxScore: 1 },`);
}
